...
class BaseDataset(Dataset):
    ...
    def _train_init(self):
        ...
        self.images = self._flatten(self.images)    # B * 3
...
class TinyDataset(Dataset):
    def __init__(self,  data_dir, device, is_train=True,white_bkgd=False):
        data = np.load(data_dir)
        self.is_train = is_train
        if is_train:
            self.n_training = 100
            self.images = data['images'][:100]  # image
            self.h, self.w = self.images.shape[1:3]
            self.camtoworlds = data['poses'][:100]  # transform_matrix
            self.focal = data['focal']  # focal
        else:
            self.n_training = 6
            self.images = data['images'][100:100+self.n_training]  # image
            self.h, self.w = self.images.shape[1:3]
            self.camtoworlds = data['poses'][100:100+self.n_training]  # transform_matrix
            self.focal = data['focal']  # focal
        self.near = 2
        self.far = 6
        self.data_dir = data_dir
...
def load_dataset(ss: Settings, device):
    # ss = Settings()
    train_set = TinyDataset(ss.filepath,device=device)
    val_set = TinyDataset(ss.filepath, device=device,is_train=False)
